    ...
    defaultConfig {
        ...
        if (localPropertiesFile.exists()) {
            try {
                ...
                println(
                    "Raw value for 'supabase.anon.key': ${
                        localProperties.getProperty("supabase.anon.key")?.take(5)
                    }..."
                ) // 只顯示前5位
            } catch (e: Exception) {
        ...
        val supabaseAnonKey =
            localProperties.getProperty("SUPABASE_ANON_KEY", "KEY_NOT_FOUND") // 使用明確的備用值

        // 打印最終用於生成 BuildConfig 的值
        println("Raw value for 'SUPABASE_URL': ${localProperties.getProperty("SUPABASE_URL")}")
        println(
            "Raw value for 'SUPABASE_ANON_KEY': ${
                localProperties.getProperty("SUPABASE_ANON_KEY")?.take(5)
            }..."
        )
